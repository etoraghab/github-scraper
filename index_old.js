...
let counter = 0;
let limit;
let globalmodel, imgTensor, filee;
...
async function getfile(counter) {
  return new Promise(async (r) => {
    if (
      ...
      // await sleep(1000);
      filee = file.createWriteStream("./github/" + counter + ".jpg");
      await http
        .get(`https://avatars.githubusercontent.com/u/${counter}`, function (
          ...
          filee.on("finish", async () => {
            ...
            await fs.readFile(`./github/${counter}.jpg`).then(async (d) => {
              imgTensor = await tf.node.decodeImage(new Uint8Array(d), 3);
